...
class KadidDataset(Dataset):
    def __init__(self, csv_path, data_path, img_size, transforms=None, crop_size=None, is_train=True):
        super().__init__()
        ...
        self.dist_img_path, self.dmos = self.csv_read(self.csv_path)

        self.x_train, self.x_test, self.y_train, self.y_test = self.train_test_split(self.dist_img_path, self.dmos, test_size=0.2, random_state=2, shuffle=True)
    ...
    def csv_read(self, csv_path):
        ...
        with open(csv_path, newline='') as f:
            ...
            for row in tqdm(reader):
                ...
                dmos.append(float(row[1]))
        return dist_img_path, dmos


    def re_size(self, dist_img, img_size):
...
if __name__ == "__main__":
    csv_path = 'C:\\Users\\yunjeongyong\\Desktop\\intern\\MANIQA-master\\data\\pipal21_val.txt'
    data_path = 'C:\\Users\\yunjeongyong\\Desktop\\intern\\MANIQA-master\\IQA_dataset\\PIPAL\\Val_Distort\\'
    dataset = KadidDataset(csv_path, data_path, img_size=256, transforms=None, is_train=False)
